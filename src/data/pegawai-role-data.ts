export type Role = 'admin' | 'verifikator_ipg' | 'verifikator_tubel' | 'pegawai'
...
export const ROLE_CONFIG: Record<Role, { label: string; bg: string; text: string; desc: string }> = {
  ...
  verifikator_tubel:{ label: 'Verifikator TUBEL',bg: 'bg-emerald-100 dark:bg-emerald-900/30',text: 'text-emerald-700 dark:text-emerald-400', desc: 'Verifikasi pengajuan TUBEL' },
  pegawai:          { label: 'Pegawai',           bg: 'bg-slate-100 dark:bg-slate-700',      text: 'text-slate-600 dark:text-slate-400',   desc: 'Mengajukan IPG / TUBEL' },
}
