      ...
      <AnimatePresence>
        {open && (
          <motion.ul
            ...
            className="absolute left-0 top-full z-50 mt-1.5 max-h-64 min-w-[10rem] overflow-y-auto rounded-xl border border-slate-200 bg-white py-1 shadow-lg [scrollbar-width:none] [&::-webkit-scrollbar]:hidden dark:border-slate-700 dark:bg-slate-800"
          >
            {options.map(opt => (
...
function StatCard({ label, value, sub, icon, accent }: {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl bg-slate-50 p-4 dark:bg-slate-800">
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-4 py-8 dark:bg-slate-950 sm:px-6 lg:px-10">
      ...
      <nav className="mb-6 flex items-center gap-1.5 text-xs text-slate-400">
        ...
        <span className="font-medium text-slate-700 dark:text-slate-200">Laporan Pengajuan</span>
      </nav>
      ...
      <div className="rounded-2xl bg-white dark:bg-slate-900">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-5">
          <div>
            <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100">Laporan Pengajuan</h2>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-600 transition hover:border-indigo-300 hover:text-indigo-600 dark:border-slate-600 dark:bg-slate-700/50 dark:text-slate-300">
          ...
          {filtered.length === 0 ? (
            ...
            <section>
              <div className="mb-3 flex items-center justify-between">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-400">Detail Pengajuan</h3>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                    {filtered.map((item) => {
                      ...
                      return (
                        <tr key={item.id} className="transition hover:bg-slate-50 dark:hover:bg-slate-800/30">
